...
class ListShouldWorkSuite extends munit.FunSuite:
  ...
  test("iter: non-consuming immutable iterator") {
    imem.withOwnership: ctx =>
      ...
      assert(peek[Int, {ctx}, {ctx}, {ctx}](list.borrowImmut(using ctx))(using ctx).map((item: imem.ImmutRef[Int, {ctx}]^{ctx}) => item.read(_ == 3)(using ctx)).getOrElse(false))
  ...
  test("iter_mut: non-consuming mutable iterator") {
    imem.withOwnership: ctx =>
      ...
      assert(pop[BoxedInteger, {ctx}, {ctx}, {ctx}](list.borrowMut(using ctx))(using ctx).map(_.borrowImmut[{ctx}, {ctx}](using ctx).read(_ == BoxedInteger(30))(using ctx)).getOrElse(false))
      assert(pop[BoxedInteger, {ctx}, {ctx}, {ctx}](list.borrowMut(using ctx))(using ctx).map(_.borrowImmut[{ctx}, {ctx}](using ctx).read(_ == BoxedInteger(20))(using ctx)).getOrElse(false))
      assert(pop[BoxedInteger, {ctx}, {ctx}, {ctx}](list.borrowMut(using ctx))(using ctx).map(_.borrowImmut[{ctx}, {ctx}](using ctx).read(_ == BoxedInteger(10))(using ctx)).getOrElse(false))
      assert(pop[BoxedInteger, {ctx}, {ctx}, {ctx}](list.borrowMut(using ctx))(using ctx).isEmpty)
  }
