...
class LinkedList[T, O1^](
  val head: imem.Box[Link[T, O1], O1]^{O1} = imem.newBoxExplicit[Link[T, O1], O1](None)
)

def newLinkedListFromBackground[T](using ctx: imem.Context^): LinkedList[T, {ctx}]^{ctx} =
    new LinkedList[T, {ctx}]()

def newLinkedListExplicit[T, O1^]: LinkedList[T, O1]^{O1} =
    new LinkedList[T, O1]()
...
def isEmptyList[T, @caps.use O1^, O2^, O3^](
  self: imem.ImmutRef[LinkedList[T, O1]^{O1}, O2]^{O2}
)(
  ...
  imem.read(self,
      list =>
        ...
        imem.read(headRef, head => head.isEmpty)
  )
...
def push[T, @caps.use O1^, O2^, O3^](
  self: imem.MutRef[LinkedList[T, O1]^{O1}, O2]^{O2},
  elem: T
)(
  ...
  if isEmptyList(imem.borrowImmut(self)) then
    imem.write(self, list => imem.setBox(list.head, Some(imem.newBoxExplicit(newNode))))
  else
...
def pop[T, @caps.use O1^, @caps.use O2^, O3^, O4^ >: {O1, O2}](
  ...
  if isEmptyList(imem.borrowImmut(self)) then None
    ...
    imem.writeBox(currentHead,
      head => head.map(nodeBox => {
        imem.write(self,
            list => imem.writeBox(nodeBox, node => imem.swapBox(node.next, list.head))
        )
...
def peek[T, @caps.use O1^, @caps.use O2^, O3^, @caps.use O4^ >: {O1, O2, O3}](
  ...
  using imem.Context^{O3}
): Option[imem.ImmutRef[T, O4]^{O4}] =
  imem.read(self,
    list =>
      val headRef = imem.borrowImmutBox[Link[T, O1], O1, {O3, O2}, O4](list.head)
      imem.read(headRef,
        head => head.map(nodeBox =>
          val nodeRef = imem.borrowImmutBox[Node[T, O1]^{O1}, O1, {O4}, O4](nodeBox)
          imem.read(nodeRef, node => imem.borrowImmutBox[T, O1, {O4}, O4](node.elem))
        )
      )
  )
...
def peekMut[T, @caps.use O1^, O2^, O3^, @caps.use O4^ >: {O1, O2, O3}](
  ...
  using ctx: imem.Context^{O3}
): Option[imem.MutRef[T, O4]^{O4}] =
  imem.write(self,
    list =>
      val headRef = imem.borrowMutBox[Link[T, O1], O1, {O3, O2}, O4](list.head)
      imem.write(headRef,
        head => head.map(nodeBox =>
            val nodeRef = imem.borrowMutBox[Node[T, O1]^{O1}, O1, {O4}, O4](nodeBox)
            imem.write(nodeRef, node => imem.borrowMutBox[T, O1, {O4}, O4](node.elem))
          )
      )
  )
